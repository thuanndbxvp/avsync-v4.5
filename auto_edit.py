...
def render_video(srt_path, img_dir, out_path, cfg=None, progress_cb=None):
    """Backward-compatible shim (Milestone 3 refactor).

    Public signature Y CHANG. Toàn bộ logic 400 dòng đã được bóc tách vào
    `services.render_service.render_video()` (Plan -> Gather Clips -> Master Concat
    -> Final). Strangler pattern: code callers (CLI main, PySide6 worker, tests)
    vẫn gọi `auto_edit.render_video(...)` y hệt.
    """
    from services.render_service import render_video as _render
    return _render(srt_path, img_dir, out_path, cfg=cfg, progress_cb=progress_cb)
